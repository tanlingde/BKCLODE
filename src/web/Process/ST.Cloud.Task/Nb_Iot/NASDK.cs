...
namespace BK.Cloud.Task
{
    public class NASDK
    {
        ...
        public NASDK(config cfg)
        {
            this.m_pltip = cfg.platformIP;
            this.m_appid = cfg.appid;
            this.m_appsecret = cfg.appkey;
            this.m_pltPort = cfg.port;
            this.m_p12certfile = cfg.p12cert;
            this.m_certpassword = cfg.certpassword;
        ...
        public TokenResult getToken()
        {
            ...
            {
                DefaultLogger.Instance.Error(ex.Message + "错误," + ex.StackTrace, ex);
                //log(ex.Message);
                //log(ex.StackTrace);
        ...
        public TokenResult GetrefreshToken(string refreshToken)
        {
            ...
            string apiPath = "/iocm/app/sec/v1.1.0/refreshToken";
            string body = "appId=" + this.m_appid + "&secret=" + this.m_appsecret + "&refreshToken=" + refreshToken;
            string method = "POST";
            string contenttype = "application/x-www-form-urlencoded";
            WebHeaderCollection headers = new WebHeaderCollection();
            try
        ...
        public string DelDevice(string token, string serverID)
        {
            string result = null;
            string apiPath = "/iocm/app/dm/v1.4.0/devices/" + serverID + "?appId="+ this.m_appid;
            string body = "";
        ...
        public RegDeviceResult regDevice(string token, string verifycode, string nodeid, string enduserid, string psk, int timeout)
        {
            ...
            string apiPath = "/iocm/app/reg/v1.1.0/deviceCredentials?appId=" + this.m_appid;
        ...
        public string modifyDevice(string token, string deviceid, string mid, string devmodel,string name)
        {
            string result = null;
            ...
            mdr.manufacturerName = mid;
            mdr.name = name;
            mdr.model = devmodel;
            mdr.deviceType = "Other";
        ...
        public HistoryDataResult queryHistoryData(string token, string deviceId, string gatewayid, string serviceId, string property, int pageno, int pagesize, string starttime, string endtime)
        {
           ...
           // "20151212T121212Z"
            starttime = "20190301T121212Z";
            endtime = DateTime.Now.ToString("yyyyMMddTHHmmssZ");
           string apiPath = string.Format("/iocm/app/data/v1.2.0/deviceDataHistory?deviceId={0}&gatewayId={1}&serviceId={2}&pageNo={3}&pageSize={4}&startTime={5}&endTime={6}", deviceId, gatewayid, serviceId, pageno, pagesize, starttime, endtime);
           // string apiPath = string.Format("/iocm/app/data/v1.2.0/deviceDataHistory?deviceId={0}&gatewayId={1}&startTime={2}&endTime={3}", deviceId, gatewayid,  "", "");
           // /iocm/app/data/v1.2.0/deviceDataHistory?deviceId={deviceId}&gatewayId={gatewayId}&appId={appId}&serviceId={serviceId}&property={property}&pageNo={pageNo}&pageSize={pageSize}&startTime={startTime}&endTime={endTime}
            string body = "";
            string method = "GET";
            string contenttype = "application/json";
            WebHeaderCollection headers = new WebHeaderCollection();
        ...
        {
            File.AppendAllText(this.m_logfile, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") +"  -  " + msg);
        }
